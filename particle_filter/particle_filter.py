...
class robot:
    ...
    # move: 
    #
    def move(self, motion, flag=1): # Do not change the name of this function
        ...
        if flag:
            ...
            dis = random.gauss(dis, self.distance_noise)
        
        if alpha > 0.001:
            beta = dis/self.length*tan(alpha)
            r = dis/beta
            ...
            x = self.x + dis*cos(self.orientation)
            y = self.y + dis*sin(self.orientation)
            theta = (self.orientation)%(2*pi)
    ...
    def sense(self, flag=1): #do not change the name of this function
        ...
        for y, x in landmarks:
            alpha = atan2(y-self.y, x-self.x)
            beta = (-self.orientation +alpha)%(2*pi)
            if flag:
                beta = random.gauss(beta, self.bearing_noise)
            Z.append(beta)
...
def particle_filter(motions, measurements, N=500): # I know it's tempting, but don't change N!
    ...
    for t in range(len(motions)):
        ...
        for r in p:
            lx.append(r.x)
            ly.append(r.y)
            lt.append(r.orientation)
        
        plt.cla()
        
        plt.plot([-80, -80, 180, 180], [-80, 180, -80, 180], '.')
        plt.plot([l[0] for l in landmarks], [l[1] for l in landmarks], 'o')
        plt.axis('equal')
        plt.plot(lx, ly, 'x')
        plt.pause(1)
