...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    # Enter code here
    #
    newpath=[e[:] for e in path]
    n=len(path)
    tol=1
    while tol>tolerance:
        tol=0
        for i in range(n):
            dx1 = (newpath[i][0]-path[i][0])
            dy1 = (newpath[i][1]-path[i][1])
        
            dx2 = -(newpath[(i+1)%n][0]+newpath[(i-1)%n][0]-2*newpath[i][0])
            dy2 = -(newpath[(i+1)%n][1]+newpath[(i-1)%n][1]-2*newpath[i][1])
            newpath[i][0]-=weight_data*dx1+weight_smooth*dx2
            newpath[i][1]-=weight_data*dy1+weight_smooth*dy2
            tol+=abs(weight_data*dx1+weight_smooth*dx2)+abs(weight_data*dy1+weight_smooth*dy2)
    for i in newpath:
        print i
    return newpath
...
plt.ylim(-1,4)
sp = smooth(path)
plt.scatter([p[0] for p in sp], [p[1] for p in sp], c='r')
plt.show()